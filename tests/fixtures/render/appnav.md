...
and ask before creating it (template: the `# My App — TODO` header, nothing else).
Other people's `TODO.*.md` files are never read, edited or worked — they are on the same
footing as `QA:`/`Admin:` rows below. Every edit to the todo file goes through the Edit tool,
...
- **Trigger: `prompt.trim()` is exactly `"done"`, optionally followed by an item id —
  `done #39`** — the whole message, trimmed, is one of those words, with or without
  ...
  hold; if `#N` is not in the file or is a `QA:`/`Admin:` row, say so and touch nothing. Either
  way: mark it `[x]`, mark every `  - [ ] QA:` sub-checkbox under it `[x]` too (the
  confirmation is that check passing), **and run `/todoIdealPrompt #N --replace`** for it in
  the same turn, without asking — the rewrite keeps the QA sub-checkboxes, now ticked, as the
  item's last sub-bullets. If more than one item is plausibly "the one", ask which — at most 3
...
usual — `done #N` names one outright. Nothing is ticked or edited by holding or by
`#go` beyond the Autoincrement pass.

Todo file format: a `# My App — TODO` header, then bullet task-list entries
...
a `#N` or `#new…` prompt, a confirm word, `/todoIdealPrompt`, `/todoIdealize` — first assign
...
Headings and `> ` lines are never items. An indented `  - [ ] QA: …` sub-checkbox under a dev
...
**App-navigation items.** When a ticket ends in a context block — the `Server:` line and the lines naming where in the app the bug lives — `/todoFromTicket` writes it as the ticket block's own first item, before the dev items: the text `/openApp` with the spec lines as sub-bullets. **An item whose text starts with `/openApp` is worked by running the `/openApp` skill with its sub-bullets as the spec**: it installs the right build, logs in and lands on the screen the ticket describes. It has no tag and no `QA:` sub-checkbox; the dev items' checks refer to it ("on the #N screen").
<!-- /prompt-todo:appNavigation -->
...
An indented `  - [ ] QA: …` sub-checkbox under a dev item (what `/todoFromTicket` writes) is
part of that item, as described under **Ticket blocks**. When `/todoIdealPrompt --replace` or
`/todoIdealize` rewrites an item, the preserved prefix is `- [state] #N KEY TAG: ` — the ticket
key and the tag survive the rewrite.

**Code markers — prompt history.** Every code change implementing todo item `#N` carries the
marker `[<name>#N]` — the file owner's `<name>` plus the id, e.g. `[jd#19]` —
...
`// Pinned to the physical top strip … [jd#16]`. A site shaped by several items
lists them all: `[jd#16 jd#21]`. This is how code is traced back to
the prompt that caused it — `grep -rn "\[jd#16\]"` finds that user's item 16.
...
**/todoIdealize.** The batch pass: rewrites every session-worked `[x]` item's text to its ideal
...
item a confirm word then refers to), and adds the `[<name>#N]` markers at that dialog's change
sites. Never duplicates an item that already exists for the dialog, never commits. It may edit

...
**App-navigation items.** When a ticket ends in a context block — the `Server:` line and the lines naming where in the app the bug lives — `/todoFromTicket` writes it as the ticket block's own first item, before the dev items: the text `/navigateApp` with the spec lines as sub-bullets. **An item whose text starts with `/navigateApp` is worked by running the `/navigateApp` skill with its sub-bullets as the spec**: it installs the right build, logs in and lands on the screen the ticket describes. It has no tag; the ticket's `QA:` items refer to it ("on the #N screen").
<!-- /prompt-todo:appNavigation -->

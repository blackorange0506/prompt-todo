...
**Code markers — off.** Code changes carry no `[<name>#N]` marker: do not add one, and leave any existing marker as it is (`/todoReverse` records the item without marking the code). `/todoMarkCode on` turns the markers back on.
<!-- /prompt-todo:codeMarkers -->

...
assert_file "RULES.md rendered"      "$D/.claude/todo-flow/RULES.md"
assert_file "config.json written"    "$D/.claude/todo-flow/config.json"
assert_file "MANIFEST written"       "$D/.claude/todo-flow/MANIFEST"
assert_file "hook copied"            "$D/.claude/hooks/todo-confirm.sh"
assert_file "skill copied"           "$D/.claude/skills/todoIdealPrompt/SKILL.md"
assert_eq "VERSION stamped" "$(cat "$ROOT/VERSION")" "$(cat "$D/.claude/todo-flow/VERSION")"
assert_eq "import line once"   "1" "$(count_in_file '@.claude/todo-flow/RULES.md' "$D/CLAUDE.md")"
...
assert_eq "config untouched on re-run" "1" "$(count_in_file 'Edited' "$D/.claude/todo-flow/config.json")"
assert_eq "package file refreshed"     "0" "$(count_in_file '# scribble' "$D/.claude/skills/todoNumber/SKILL.md")"
assert_eq "import line still once"     "1" "$(count_in_file '@.claude/todo-flow/RULES.md' "$D/CLAUDE.md")"
...
assert_contains "re-rendered with the edited title" "$(cat "$D/.claude/todo-flow/RULES.md")" '# Edited — TODO'

# --force resets config with a backup
out="$(bash "$ROOT/install.sh" --target "$D" --force 2>&1)"
assert_file "config backup" "$D/.claude/todo-flow/config.json.bak"
assert_eq "config reset" "0" "$(count_in_file 'Edited' "$D/.claude/todo-flow/config.json")"
...
assert_no_file "todo-flow dir gone"   "$D/.claude/todo-flow"
...
assert_eq "import line removed" "0" "$(count_in_file '@.claude/todo-flow/RULES.md' "$D/CLAUDE.md")"

...
3. **For each session-worked done item**, distill the ideal prompt following the rules in the
   ...
   as indented `  - ` sub-bullets, a ticked `  - [x] QA:` sub-checkbox kept as the last
   sub-bullet. Open (`[ ]`) items are never rewritten.

4. **Reply with a summary**: per rewritten item a compact `#N: old text → new first line`, and

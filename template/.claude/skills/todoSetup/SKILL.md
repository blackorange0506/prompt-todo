...
> traced back to the prompt that caused it. The project title only appears in the file
> headers.
>
...
Show the current `tags.list` as a table (tag, meaning), the `default`, and the `ignore` rows.
Explain in two lines: a tag goes right after the id (`- [ ] #19 IOS: …`) and scopes the item;
ignore rows are the user's own tasks, never touched by Claude.

**Ask** (loop until *Keep as is* / *Skip*): *Remove rows*, *Add a row* (tag + one-line
meaning; letters, digits, `+`, no colon), *Change the default* (one of the list, or *none*),
*Change the ignore rows*, *Keep as is*. Validate before writing: the default must be in the
list, a tag cannot be in both lists; `render_rules.py --check` says so too.

**Writes:** `tags`. Re-render. `--yes`: unchanged.
...
> ```
>
> Then `#33` works the item, `works` ticks it and its QA check and rewrites it to its ideal
> prompt.
...
1. The effective config as a short table: title, git name → todo file (exists / missing),
   tags (count + default + ignore rows), confirm words, tracker, app navigation, attachments
   directory.

...
where `<name>` is the output of `git config user.name` (e.g. `{{userExample}}` →
`TODO.{{userExample}}.md`). Resolve it once per session. If the file does not exist, say so
and ask before creating it (template: the `# {{projectTitle}} — TODO` header, nothing else).
Other people's `TODO.*.md` files are never read, edited or worked — they are on the same
footing as {{ignoreRows}} rows below. Every edit to the todo file goes through the Edit tool,
...
- **Trigger: `prompt.trim()` is exactly {{confirmWords}}, optionally followed by an item id —
  `{{confirmFirst}} #39`** — the whole message, trimmed, is one of those words, with or without
  ...
  hold; if `#N` is not in the file or is a {{ignoreRows}} row, say so and touch nothing. Either
...
usual — `{{confirmFirst}} #N` names one outright. Nothing is ticked or edited by holding or by
`#go` beyond the Autoincrement pass.

Todo file format: a `# {{projectTitle}} — TODO` header, then bullet task-list entries
...
hand are kept as written. Ids are scoped to the user — `#19` in `TODO.{{userExample}}.md` and
...
`- [ ] #32 {{ticketExample}} IOS: description` — and the file may group a ticket's items under
a `## {{ticketExample}} — <summary>` heading with an optional `> ` excerpt line beneath it.
...
marker `[<name>#N]` — the file owner's `<name>` plus the id, e.g. `[{{userExample}}#19]` —
...
`// Pinned to the physical top strip … [{{userExample}}#16]`. A site shaped by several items
lists them all: `[{{userExample}}#16 {{userExample}}#21]`. This is how code is traced back to
the prompt that caused it — `grep -rn "\[{{userExample}}#16\]"` finds that user's item 16.
